CREATE DATABASE IF NOT EXISTS `cemcsmfb_db`;
USE `cemcsmfb_db`;
...
CREATE TABLE `follow_up_logs` (
  ...
  INDEX `idx_stage_followup` (`stage`)
);
...
CREATE TABLE `managed_pages` (
  `id` INT AUTO_INCREMENT PRIMARY KEY,
  `slug` VARCHAR(255) UNIQUE NOT NULL COMMENT 'URL slug e.g. /about or /blog/post-title',
  `title` VARCHAR(255) NOT NULL,
  `meta_description` VARCHAR(320),
  `content` LONGTEXT NOT NULL COMMENT 'HTML content for the page body',
  `status` ENUM('draft', 'published') NOT NULL DEFAULT 'draft',
