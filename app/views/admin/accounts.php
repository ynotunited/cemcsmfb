<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:var(--space-5);">
    <h2 style="margin:0;color:var(--blue-dark);">Account Applications</h2>
        ...
        <table style="width:100%;border-collapse:collapse;text-align:left;min-width:1000px;">
            <thead>
                <tr style="border-bottom:2px solid var(--neutral-100);background:var(--neutral-100);">
                    ...
                    <th style="padding:var(--space-3) var(--space-4);font-size:0.8rem;color:var(--neutral-500);">Applicant</th>
                    <th style="padding:var(--space-3) var(--space-4);font-size:0.8rem;color:var(--neutral-500);">Email</th>
                    <th style="padding:var(--space-3) var(--space-4);font-size:0.8rem;color:var(--neutral-500);">Phone</th>
                    <th style="padding:var(--space-3) var(--space-4);font-size:0.8rem;color:var(--neutral-500);">Date</th>
            ...
            <tbody>
                <?php foreach ($applications as $app): ?>
                    <tr style="border-bottom:1px solid var(--neutral-100);">
                        <td style="padding:var(--space-3) var(--space-4);font-weight:600;color:var(--blue-primary);">ACC-<?= str_pad($app['id'], 4, '0', STR_PAD_LEFT) ?></td>
                        <td style="padding:var(--space-3) var(--space-4);"><?= htmlspecialchars($app['full_name']) ?></td>
                        <td style="padding:var(--space-3) var(--space-4);"><?= htmlspecialchars($app['email']) ?></td>
                        <td style="padding:var(--space-3) var(--space-4);"><?= htmlspecialchars($app['phone']) ?></td>
                        <td style="padding:var(--space-3) var(--space-4);"><?= date('M j, Y', strtotime($app['created_at'])) ?></td>
                        ...
                        <td style="padding:var(--space-3) var(--space-4);">
                            <div style="display:flex;gap:8px;align-items:center;">
                                <a class="btn btn-outline" style="padding:4px 10px;font-size:0.75rem;" href="<?= APP_URL ?>/admin/application?type=account&id=<?= (int) $app['id'] ?>">View</a>
                                <form method="POST" action="<?= APP_URL ?>/admin/accounts/status" style="display:flex;gap:6px;align-items:center;">
                                    <?= CsrfHelper::field() ?>
